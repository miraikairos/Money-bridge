...
function roleInDeal(deal, userId) {
  ...
  if (String(deal.creator._id) === uid) return deal.creatorRole; // 'buyer' | 'seller'
...
const dealToJson = (d) => ({
  ...
  dealType: d.dealType,
  status: d.status,
  creator: { _id: d.creator._id, name: d.creator.name, email: d.creator.email },
...
// POST /api/deals/create  (multipart optional 'file' = reference/portfolio)
exports.createDeal = async (req, res, next) => {
  try {
    ...
    const creatorRole = req.user.role || req.body.role; // 'buyer' | 'seller' (from auth user)
...
exports.getDeals = async (req, res, next) => {
  try {
    ...
    if (scope === 'mine') {
      ...
      // available = open deals created by OTHER users
      q.status = 'open';
      q.creator = { $ne: uid };
    }
...
exports.getDealById = async (req, res, next) => {
  try {
    ...
    // Open deals are viewable by anyone (needed for browsing); active deals only by participants
...
exports.agreeDeal = async (req, res, next) => {
  try {
    ...
    if (deal.buyerAgreed && deal.sellerAgreed) {
      deal.status = 'sample_pending';
    } else {
      deal.status = 'waiting_for_agreement';
    }
    await deal.save();
    const fresh = await Deal.findById(deal._id).populate(POPULATE, 'name email');
    res.json({
      message: deal.status === 'sample_pending' ? 'Both parties agreed — seller can upload sample'
...
// POST /api/deals/:id/sample  (multipart 'file') — SELLER ONLY
...
exports.approveSample = async (req, res, next) => {
  try {
    ...
    deal.status = 'waiting_for_funding'; // Member 3 takes over from here
    await deal.save();
    const fresh = await Deal.findById(deal._id).populate(POPULATE, 'name email');
...
// POST /api/deals/:id/final-file  (multipart 'file') — SELLER ONLY
...
exports.acceptDelivery = async (req, res, next) => {
  try {
    ...
    deal.status = 'ready_for_payment_release'; // Member 3 releases payment from here
    await deal.save();
    const fresh = await Deal.findById(deal._id).populate(POPULATE, 'name email');
...
// ==========================================================
// INTEGRATION POINTS FOR MEMBER 3 (no auth required by design)
// Secure these with an internal key before production.
// ==========================================================

// PUT /api/deals/:id/funding-status   body: { paymentStatus: "funded" | "not_funded", paymentRef? }
...
// PUT /api/deals/:id/payment-release   body: { paymentRef? } — called after Member 3 releases funds
...
// PUT /api/deals/:id/cancel — participants only, workflow rules enforced
  ...
  try {
    ...
    if (!CANCELLABLE.includes(deal.status)) {
      return res.status(400).json({
        message: `Deal cannot be cancelled at status "${deal.status}" — use dispute/payment module`
      });
    }
